package com.h.cheng.chain100.view;
...
public class StateView extends View {
    ...
    public StateView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (mEmptyResource == 0) {
            mEmptyResource = R.layout.no_data_center;
        }
        if (mRetryResource == 0) {
    ...
    public View showEmpty(final Refresh refresh) {
        if (mEmptyView == null) {
            mEmptyView = inflate(mEmptyResource);
            mEmptyView.findViewById(R.id.tv_refresh).setOnClickListener(new OnClickListener() {
                @Override
    ...
    public View showEmpty() {
        if (mEmptyView == null) {
            mEmptyView = inflate(mEmptyResource);
            mEmptyView.findViewById(R.id.tv_refresh).setVisibility(GONE);
        }
        showView(mEmptyView);
        return mEmptyView;
    }

    public View showEmptyV2(final Refresh refresh) {
    ...
    public View showError(final Refresh refresh) {

        mEmptyView = inflate(mEmptyResource);
        TextView tv_msg = (TextView) mEmptyView.findViewById(R.id.tv_msg);
        tv_msg.setText("网络加载失败，请点击重试");
        mEmptyView.findViewById(R.id.tv_refresh).setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                showLoading();
    ...
    public View showEmpty(final Refresh refresh, String msg) {
        if (mEmptyView == null) {
            mEmptyView = inflate(mEmptyResource);
            TextView textView = setTextMsg(msg);
    ...
    public View showLogin(final Refresh refresh, String msg) {
        if (mEmptyView == null)
            mEmptyView = inflate(mEmptyResource);
        TextView textView = mEmptyView.findViewById(R.id.tv_refresh);
        TextView tv_msg = mEmptyView.findViewById(R.id.tv_msg);
        tv_msg.setText(msg);
        textView.setText("登录");
        textView.setTextColor(0xffff7e64);
    ...
    @NonNull
    private TextView setTextMsg(String msg) {
        TextView textView = mEmptyView.findViewById(R.id.tv_refresh);
        TextView tv_msg = mEmptyView.findViewById(R.id.tv_msg);
        textView.setText(msg);
        if (msg.equals("登录")) {
            tv_msg.setText("您还未登录啊 T.T");
        } else if (msg.equals("")) {
            tv_msg.setText("您还未登录啊 T.T");
        } else {
            tv_msg.setText("这里居然什么都没有啊 T.T");
        }
        return textView;
    }
    ...
    public View showLoading() {
        if (mLoadingView == null) {
            ...
            ImageView loadingAnimImg = mLoadingView.findViewById(R.id.base_loading_img);
            startAnim(loadingAnimImg);
        }
